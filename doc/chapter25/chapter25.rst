...
Flash芯片上读写数据。如需要记录本书的书名“零死角玩转STM32-H743系列”，可以把这些文字转化成ASCII码，存储在数组中，然后调用SPI_FLASH_BufferWrite函数，把数组内容写入到SPI
...
2) 使用KEIL软件打开工程文件(..\SPI—FatFs文件系统\Project\RVMDK(uv5)\\
   Fire-H743V.uvprojx)，并将FatFs组件文件添加到工程中，需要添加有ff.c、diskio.c和cc936.c三个文件，见
   图25_7_。
